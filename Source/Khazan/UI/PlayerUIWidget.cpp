...
#pragma region Blueprint 에서 생성해준 widget
	...
	/* Stamina 관련 */
	m_pWidgetStaminProgressBar = Cast<UPlayerStaminaProgressBarWidget>(GetWidgetFromName(TEXT("WBP_StaminaBar")));
	ensureAlways(m_pWidgetHpProgressBar);

	m_pCurrentStaminaText = Cast<UTextBlock>(GetWidgetFromName(TEXT("Current_Stamina_Text")));
	ensureAlways(m_pCurrentStaminaText);
	/* ----------------- */
...
{
	// 일단 TEXT 부터 
	m_pCurrentHpText->SetText(FText::AsNumber(_iCurrentHp));
}
...
{
	m_pWidgetStaminProgressBar->Set_Up_Stamina(_fCurrentStamina, _fMaxStamina);
}
...
void UPlayerUIWidget::UpdateProgressBarStamina(float _fCurrentStamina, float _fMaxStamina)
{
	...
	{
		if (m_bHasPlayedStaminaRenderOpacity == false)
		{
			PlayAnimation(Render_Opacity_StaminaProgressBar);
			PlayAnimation(Render_Opactiy_StaminaText);
			m_bHasPlayedStaminaRenderOpacity = true;
		}
	}

	else
	{
		m_bHasPlayedStaminaRenderOpacity = false;
		m_pWidgetStaminProgressBar->SetRenderOpacity(1.0f);
		m_pCurrentStaminaText->SetRenderOpacity(1.0f);
	}

	/* ------------------------------------------- */


	int DisplayStamina = _fCurrentStamina;

	m_pCurrentStaminaText->SetText(FText::AsNumber(DisplayStamina));
	m_pWidgetStaminProgressBar->UpdateProgressBar(_fCurrentStamina, _fMaxStamina);
}
